...
# 제목
st.title("📊 코인 시뮬레이션")
st.markdown("<div style='margin-bottom:10px; color:gray;'>※ 점선=신호 흐름선 · 성공 시 도달 캔들의 <b>고가(high) 바로 위</b>에 ⭐ 표시</div>", unsafe_allow_html=True)
...
with c6:
    rsi_mode = st.selectbox("RSI 조건", ["없음", "≤", "≥"], index=0)  # 1단위 세밀
...
def first_bull_pair_start_idx(df, start_i, end_i):
    """연속 양봉(두 캔들 모두 종가>시가 & 종가 연속 상승) 최초 시작 인덱스"""
    if start_i + 2 > end_i:
        return None
...
def simulate(df, rsi_mode, rsi_level, lookahead, thr_pct, bb_cond, dedup_mode,
    ...
    while i < n:
        if i in base_sig_idx:
            ...
            end_time = df.at[end, "time"]; end_close = float(df.at[end, "close"])
            ...
            if max_ret >= thr and not closes.empty:
                ...
                if not first_hit.empty:
                    ...
                    end_close = float(df.at[int(idx_hit[0]), "close"]) if len(idx_hit) > 0 else float(first_hit.iloc[0]["close"])
                result = "성공"
            elif final_ret < 0:
                result = "실패"
            ...
            res.append({
                "신호시간": df.at[i, "time"],              # 1차 조건 충족 시각
                "시각_라인시작": df.at[anchor_i, "time"],   # 점선/마커 시작 시각(양봉조건 시 이동)
                "기준시가": int(round(base)),
                "종료시간": end_time, "종료가": end_close,
...
try:
    ...
    # 양봉 조건 사용 시 BB 미설정 금지
    if (bb_cond == "없음") and use_bull2:
        st.error("볼린저밴드 설정이 없음 상태입니다")
        st.stop()

    # 조건 요약 (RSI 상승/하락 용어, 봉 종류 포함)
    if rsi_mode == "없음": rsi_txt = "RSI 없음"
    elif rsi_mode == "≤":  rsi_txt = f"RSI 하락 ≤ {int(rsi_level)}"
    else:                  rsi_txt = f"RSI 상승 ≥ {int(rsi_level)}"
    ...
    res_all   = simulate(df, rsi_mode, rsi_level, lookahead, threshold_pct, bb_cond,
                         "중복 포함", minutes_per_bar, market_code, bb_window, bb_dev,
    ...
    res_dedup = simulate(df, rsi_mode, rsi_level, lookahead, threshold_pct, bb_cond,
                         "중복 제거", minutes_per_bar, market_code, bb_window, bb_dev,
                         use_bull2=use_bull2, allow_other_secondary=allow_other_secondary)
    ...
    if not res.empty:
        # 시작 마커(원) - 양봉 사용 시에도 라인 시작(anchor) 시각/가격 기준
        for _label, _color in [("성공","red"), ("실패","blue"), ("중립","#FF9800")]:
            sub = res[res["결과"] == _label]
            if sub.empty: continue
            fig.add_trace(go.Scatter(
                x=sub["시각_라인시작"], y=sub["기준시가"],
                mode="markers", name=f"신호({_label})",
                marker=dict(size=9, color=_color, symbol="circle",
        ...
        for _, row in res.iterrows():
            start_x = pd.to_datetime(row["시각_라인시작"])  # 점선 시작: anchor
            ...
            # 중복 포함: 모든 선 그림(범례는 그룹당 1개)
            # 중복 제거: 첫 선만 토글용(범례 1개), 나머지는 숨기고 싶으면 여기서 제외하면 됨.
            show_this = True if dup_mode.startswith("중복 포함") else (not legend_emitted[grp])
            if show_this:
                fig.add_trace(go.Scatter(
    ...
    # RSI (secondary y with shapes for 30/70)
    ...
    fig.add_shape(type="line", x0=t0, x1=t1, y0=70, y1=70, xref="x", yref="y2",
                  line=dict(dash="dash", color="#E63946", width=1.1))
    fig.add_shape(type="line", x0=t0, x1=t1, y0=30, y1=30, xref="x", yref="y2",
    ...
    if not res.empty:
        ...
        def fmt_hhmm(s_str, e_str):
            try:
                s = pd.to_datetime(s_str); e = pd.to_datetime(e_str)
        ...
        ordered = ["신호시간","기준시가","RSI(13)","성공기준(%)","결과","최종수익률(%)","최저수익률(%)","최고수익률(%)","도달시간"]
        show_cols = [c for c in ordered if c in tbl.columns]
        tbl = tbl[show_cols]
