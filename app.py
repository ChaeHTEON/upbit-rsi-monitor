...
# ===== 시뮬레이션 (중복 포함/제거) =====
        ...
        # ✅ 거래량 기준선 NaN 방지 + 초기구간 표시 개선 (df_plot 기준 통일)
        if "vol_mean" not in df_plot.columns:
            df_plot["vol_mean"] = df_plot["volume"].rolling(20, min_periods=1).mean().fillna(method="bfill").fillna(method="ffill")
        if "vol_threshold" not in df_plot.columns:
            df_plot["vol_threshold"] = df_plot["vol_mean"] * 2.5
        fig.add_trace(
            go.Scatter(
                x=df_plot["time"], y=df_plot["vol_threshold"],
        ...
        # ✅ 추가: 2.5배선 돌파 + 음봉일 때, 거래량 막대 위(보조지표 패널)에 별표 표시 (df_plot 기준)
        try:
            cond_vol_spike = (df_plot["volume"] >= df_plot["vol_threshold"]) & (df_plot["close"] < df_plot["open"])
            if cond_vol_spike.any():
                fig.add_trace(
                    go.Scatter(
                        x=df_plot.loc[cond_vol_spike, "time"],
                        y=df_plot.loc[cond_vol_spike, "volume"] * 1.05,  # 막대 꼭대기 약간 위
                        mode="markers",
                        name="거래량★(2.5배·음봉)",
                        marker=dict(
                            size=11,
