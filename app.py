...
def simulate(df, rsi_side, lookahead, thr_pct, bb_cond, dedup_mode):
    ...
    if "≤" in rsi_side: sig_idx=df.index[(df["RSI13"].notna())&(df["RSI13"]<=30)].tolist()
    else: sig_idx=df.index[(df["RSI13"].notna())&(df["RSI13"]>=70)].tolist()
    for i in sig_idx:
        end=i+lookahead
        if end>=n: continue
        if bb_cond!="없음":
...
try:
    ...
    def _summarize(df):
        ...
        total=len(df); succ=int((df["결과"]=="성공").sum()); fail=int((df["결과"]=="실패").sum()); neu=int((df["결과"]=="중립").sum())
        win=succ/total*100.0; range_sum=float((df["최고수익률(%)"]-df["최저수익률(%)"]).sum())
        final_succ=float(df.loc[df["결과"]=="성공","최종수익률(%)"].sum()); final_fail=float(df.loc[df["결과"]=="실패","최종수익률(%)"].sum())
    ...
    for label,data in [("중복 포함 (연속 신호 모두)",res_all),("중복 제거 (연속 동일 결과 1개)",res_dedup)]:
        ...
        c7.metric("최종수익률 합계",f"{(final_succ+final_fail):.1f}%"); st.markdown("---")
    ...
    fig.update_layout(title=f"{market_label.split(' — ')[0]} · {tf_label} · RSI(13) + BB 시뮬레이션",
                      xaxis_rangeslider_visible=False,height=600,autosize=False,legend_orientation="h",legend_y=1.05,
