    ...
    while i < n:
        if i in base_sig_idx:
            ...
            if sec_cond == "양봉 2개 연속 상승":
                ...
                # B1: 전체에서 50% 이상 진입한 '첫 양봉'
                B1_idx, B1_close = None, None
                for j in range(n):
                    if b1_pass(j):
                        B1_idx = j; B1_close = float(df.at[j, "close"]); break
                ...
                # T: B3 이후 종가 ≥ B1 종가인 첫 캔들 → 여기서 신호 시작
                T_idx = None
            ...
            if not first_hit.empty:
                ...
                end_time = hit_time; end_close = target_price
                result = "성공"; final_ret = thr
            else:
                if final_ret >= thr: result = "성공"; final_ret = thr
