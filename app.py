...
def simulate(
    ...
    # 보조: B1 50% 진입 판단(양봉 한정, ref는 'i' 시점 BB선)
    def b1_pass(i, j):
        # j: 후보 캔들(양봉)
        if not is_bull(j):
            return False
    ...
    while i < n:
        if i in base_sig_idx:
            ...
            if sec_cond == "양봉 2개 연속 상승":
                # entry_idx, entry_idx+1 두 개 캔들 연속 양봉 + 상승
                if entry_idx + 1 <= n-1:
                ...
                # B2, B3: B1 이후 양봉 2개(연속성 무관), 단 '측정 캔들 수' 내에서
                bull_count = 0
                B3_idx = None
                # B1 이후 lookahead개 범위까지 검사
                ...
                # T: B3 이후부터 close ≥ B1_close 를 처음 만족하는 캔들
                T_idx = None
