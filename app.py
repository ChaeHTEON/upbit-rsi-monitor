...
try:
    ...
    # -----------------------------
    # 🔄 차트 컨트롤 (차트 근처, UI/UX 유지)
    # -----------------------------
    if "last_refresh" not in st.session_state:
        st.session_state["last_refresh"] = datetime.now()
    st.markdown("<div class='section-title'>차트 컨트롤</div>", unsafe_allow_html=True)
    cc1, cc2 = st.columns([1,2])
    with cc1:
        if st.button("🔄 새로고침", key="chart_refresh_btn"):
            now = datetime.now()
    ...
    with cc2:
        # 상단과 동일 기능의 종목 선택 (동기화)
        sel_idx = next((i for i, (_, code) in enumerate(MARKET_LIST) if code == market_code), 0)
        market_label2, market_code2 = st.selectbox(
            "차트 근처 종목 선택", MARKET_LIST, index=sel_idx, format_func=lambda x: x[0], key="chart_market_select"
        )
        if market_code2 != market_code:
            # 선택값 반영 후 전체를 재실행(뷰 상태는 uirevision으로 유지)
            st.session_state["chart_selected_market"] = market_code2
            st.experimental_rerun()

    # 상단 select와 차트 근처 select 동기화 적용
    if "chart_selected_market" in st.session_state and st.session_state["chart_selected_market"] != market_code:
        _, market_code = next((item for item in MARKET_LIST if item[1] == st.session_state["chart_selected_market"]), (market_label, market_code))
    ...
    fig.update_layout(
        ...
        uirevision="chart-view"  # ✅ 줌/스크롤/뷰 상태 유지
    )
