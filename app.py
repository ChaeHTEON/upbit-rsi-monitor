...
def simulate(
    ...
    if rsi_mode != "없음" and bb_cond != "없음":
        ...
        # 1차 조건 없음 → 2차 조건 단독 허용
        base_sig_idx = list(range(n))

    # 2차 조건 함수
    def bullish_two_in_a_row_from(idx):
        # idx, idx+1 두 캔들이 모두 양봉이고 두 번째 종가 > 첫 번째 종가
        if idx+1 > n-1:
            return False
        c0, o0 = float(df.at[idx,"close"]), float(df.at[idx,"open"])
        c1, o1 = float(df.at[idx+1,"close"]), float(df.at[idx+1,"open"])
        return (c0 > o0) and (c1 > o1) and (c1 > c0)

    def bb_first_bull_entry(i):
        # 조건 충족 캔들 i 이후 첫 양봉(= i+1)의 '기준선 50% 이상 진입' 판정
        if bb_cond == "없음":
            return None
        if i+1 > n-1:
            return None
        if bb_cond == "상한선":
            ref = float(df.at[i,"BB_up"])
        elif bb_cond == "중앙선":
            ref = float(df.at[i,"BB_mid"])
        elif bb_cond == "하한선":
            ref = float(df.at[i,"BB_low"])
        else:
            return None
        if pd.isna(ref):
            return None
    ...
    while i < n:
        if i in base_sig_idx:
            entry_idx = i + 1  # ***진입 캔들: 조건 충족 다음 캔들***
            if entry_idx >= n:
                break

            # 2차 조건 처리 + 진입가 결정
            if sec_cond == "양봉 2개 연속 상승":
                if not bullish_two_in_a_row_from(entry_idx):
                    i += 1
                    continue
                base = float(df.at[entry_idx,"close"])  # 진입가 = 진입 캔들의 종가
                ...
                if not bullish_two_in_a_row_from(entry_idx):
                    i += 1
                    continue
                base = float(buy_price)  # 진입가 = 첫 양봉 종가(조건 충족)
            else:
                base = float(df.at[entry_idx,"close"])  # 기본 진입가

            signal_time = df.at[entry_idx,"time"]      # ***신호시간 = 진입 캔들 시간***
            ...
            end_close = float(df.at[end, "close"]) if not closes.empty else base
...
try:
    ...
    if not res.empty:
        ...
        for _, row in res.iterrows():
            ...
            if grp == "성공":
                ...
                fig.add_trace(go.Scatter(
                    x=[end_x], y=[end_close], mode="markers",
                    name=f"도착-{grp}",
    ...
    if not res.empty:
        ...
        # 도달시간(분:초) 표기
        tbl["도달시간"] = [
            fmt_hhmm(res.loc[i,"신호시간"], res.loc[i,"종료시간"]) for i in range(len(res))
        ]
        if "도달분" in tbl:
        ...
        def style_result(val):
            ...
            elif val == "실패": return "color: #1E40AF;"
            elif val == "중립": return "color: #FF9800;"
            return ""
