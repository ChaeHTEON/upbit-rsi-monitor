...
def simulate(df, rsi_mode, rsi_low, rsi_high, lookahead, thr_pct, bb_cond, dedup_mode,
    ...
    res=[]; n=len(df); thr=float(thr_pct)
        ...
        if i in base_sig_idx:
            ...
            if sec_cond == "양봉 2개 연속 상승":
                if entry_idx + 1 <= n-1:
                    ...
                    if not ((c0>o0) and (c1>o1) and (c1>c0)): i += 1; continue
                else:
                    i += 1; continue

            elif sec_cond == "BB 기반 첫 양봉 50% 진입":
                # B1: 전체에서 자기자신 BB 기준 50% 이상 진입한 첫 양봉
                B1_idx = None; B1_close = None
                for j in range(i+1, n):
                    if b1_pass(j):
                        B1_idx   = j
                        B1_close = float(df.at[j,"close"])
                        break
                if B1_idx is None: i += 1; continue

                # B2,B3: B1 이후 양봉 2개(연속성 무관), B1 이후 lookahead 이내
                bull_count = 0; B3_idx = None
...
try:
    ...
    if rsi_mode == "없음": rsi_txt = "없음"
    elif rsi_mode == "현재(과매도/과매수 중 하나)": rsi_txt = f"현재: (과매도≤{int(rsi_low)}) 또는 (과매수≥{int(rsi_high)})"
    elif rsi_mode == "과매도 기준": rsi_txt = f"과매도≤{int(rsi_low)}"
    elif rsi_mode == "과매수 기준": rsi_txt = f"과매수≥{int(rsi_high)}"
    else: rsi_txt = "없음"
    ...
    def _summarize(df_in):
        ...
        total = len(df_in); succ = (df_in["결과"]=="성공").sum()
        fail  = (df_in["결과"]=="실패").sum(); neu = (df_in["결과"]=="중립").sum()
    ...
    for label, data in [("중복 포함 (연속 신호 모두)",res_all),("중복 제거 (연속 동일 결과 1개)",res_dedup)]:
        ...
        m6.markdown(f"<div style='font-weight:600;'>최종수익률 합계: "
                    f"<span style='color:{col}; font-size:1.1rem'>{total_final:.1f}%</span></div>", unsafe_allow_html=True)
    ...
    if not res.empty:
        ...
        for _, row in res.iterrows():
            ...
            fig.add_trace(go.Scatter(x=[start_x,end_x], y=[start_y,end_close], mode="lines",
            ...
            if grp == "성공":
                ...
                fig.add_trace(go.Scatter(x=[end_x], y=[star_y], mode="markers", name="목표 도달",
                                         marker=dict(size=15,color="orange",symbol="star",line=dict(width=1,color="black")),
                                         showlegend=False))
            else:
                fig.add_trace(go.Scatter(x=[end_x], y=[end_close], mode="markers", name=f"도착-{grp}",
