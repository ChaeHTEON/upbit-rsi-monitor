...
def get_upbit_krw_markets():
    ...
    try:
        ...
        rows = []
        for it in items:
            if it.get("market","").startswith("KRW-"):
                sym = it["market"][4:]
                label = f'{it.get("korean_name","")} ({sym}) — {it["market"]}'
...
def simulate(df, rsi_side, lookahead, thr_pct, bb_cond, dedup_mode,
    ...
    ...
    # (생략: 기존 simulate 함수 그대로 유지, 앞서 안내한 수정사항 반영)
    ...
...
try:
    ...
    # ✅ 조건 먼저 확인: 둘 다 '없음'이면 외부 호출 없이 종료
