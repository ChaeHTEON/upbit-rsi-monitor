...
def fetch_upbit_paged(market_code, interval_key, start_dt, end_dt, minutes_per_bar):
    ...
    df = pd.DataFrame(all_data).rename(
        columns={
...
try:
    ...
    if not res.empty:
        ...
        def color_result(val):
            if val == "성공":
                return "color:red; font-weight:600; background-color:#FFFACD;"
            if val == "실패":
                return "color:blue;"
