...
def main():
    ...
    try:
        ...
        # 📘 1% 메인 전략 안내 (매매기법 요약)
        # -----------------------------
        with st.expander("📘 하루 1% 수익 전략 (9개 매매기법 안내)", expanded=False):
            st.markdown("""
...
"""
        ...
        if st.session_state["alert_history"]:
            for h in st.session_state["alert_history"]:
                ...
                🕒 **{h['time']}**
                # === 전략별 표시 시간대 자동 구분 ===
                if strategy_name in ["4D_Sync", "240m_Sync", "LCT"]:
                    display_tf = "4시간"
                else:
                    display_tf = f"{tf}분"

                st.markdown(f"- 종목: {s_code} ({display_tf})")
                st.markdown(f"- 전략: {strategy_name} (N/A)")
                st.markdown(f"- 예상 매수가: 계산중...")
                st.markdown(f"- 성공 확률(예상): <span style='color: green;'>0.0%</span>", unsafe_allow_html=True)
                ---
                """)
        else:
