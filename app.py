...
def fetch_upbit_paged(market_code, interval_key, start_dt, end_dt, minutes_per_bar):
    ...
    while True:
        if len(all_data) >= max_calls * req_count:
            break
...
def simulate(df, rsi_side, lookahead, thr_pct, bb_cond, dedup_mode,
    ...
    def bb_ok(i: int) -> bool:
        ...
        if bb_cond == "하한선 하향돌파":
            return pd.notna(lo) and ((lo_px <= lo <= hi) or cl <= lo)
        if bb_cond == "하한선 상향돌파":
            if pd.isna(lo): return False
            prev_cl = float(df.at[i-1,"close"]) if i > 0 else None
            return (lo_px <= lo <= hi) or (prev_cl is not None and prev_cl < lo and cl >= lo)
        if bb_cond == "상한선 하향돌파":
            return pd.notna(up) and ((lo_px <= up <= hi) or cl <= up)
        if bb_cond == "상한선 상향돌파":
            return pd.notna(up) and ((lo_px <= up <= hi) or cl >= up)
        if bb_cond == "하한선 중앙돌파":
            return pd.notna(mid) and ((lo_px <= mid <= hi) or cl >= mid)
        if bb_cond == "상한선 중앙돌파":
            return pd.notna(mid) and ((lo_px <= mid <= hi) or cl <= mid)
        return True

    if bb_cond != "없음":
        bb_idx = [i for i in df.index if bb_ok(i)]
    else:
        bb_idx = df.index.tolist()

    if rsi_side == "없음" and bb_cond != "없음":
        sig_idx = bb_idx
    elif bb_cond == "없음" and rsi_side != "없음":
        sig_idx = rsi_idx
    else:
        sig_idx = sorted(set(rsi_idx) | set(bb_idx))
    # ----- [교체 후 끝] -----

    # 이후 부분은 동일 (수익률 계산, 결과 판정 등)
    for i in sig_idx:
        end=i+lookahead
        if end>=n: continue
