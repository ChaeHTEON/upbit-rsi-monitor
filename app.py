...
def simulate(df, rsi_side, lookahead, thr_pct, bb_cond, dedup_mode):
    """
    - 신호시간: datetime 그대로 보관 (차트용 정확도 유지)
    - 도달시간: '도달분'(int, 경과 분)으로 저장 → 표에서 HH:MM로 포맷
    """
    res=[]
...
try:
    ...
    for lw, alpha in [(6,0.05),(4,0.1),(2,0.2),(1.2,1.0)]:
        fig.add_trace(go.Scatter(
            ...
            opacity=alpha, name="RSI(13)" if lw==1.2 else "",
            yaxis="y2", showlegend=(lw==1.2)
        ))
