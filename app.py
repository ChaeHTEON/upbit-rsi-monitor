try:
    ...
    if st.button("시뮬레이션 실행"):
        ...
        if df.empty:
            ...
            res_all = simulate(
                df, rsi_side, lookahead, threshold_pct, bb_cond, dup_mode,
                minutes_per_bar, market_code, bb_window, bb_dev
            )
            ...
            fig.add_trace(go.Scatter(x=df["time"], y=df["BB_up"],  mode="lines", name="BB 상단",
                                     line=dict(color="#FFB703", width=1.5), connectgaps=True))
            fig.add_trace(go.Scatter(x=df["time"], y=df["BB_low"], mode="lines", name="BB 하단",
                                     line=dict(color="#219EBC", width=1.5), connectgaps=True))
            fig.add_trace(go.Scatter(x=df["time"], y=df["BB_mid"], mode="lines", name="BB 중앙",
                                     line=dict(color="#8D99AE", width=1.2, dash="dot"), connectgaps=True))
            # RSI 라벨용: 보조축 없이 점선만
            fig.add_trace(go.Scatter(x=df["time"], y=df["RSI13"], mode="lines",
                                     name="RSI(13)", line=dict(color="#2b8a3e", width=1, dash="dot"),
