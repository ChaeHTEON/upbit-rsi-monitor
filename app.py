...
def simulate(df, rsi_mode, rsi_low, rsi_high, lookahead, thr_pct, bb_cond, dedup_mode,
        ...
        if sec_cond == "양봉 2개 연속 상승":
            ...
            for j in range(i0 + 1, scan_end + 1):
                if manual_supply_levels:
                    prev_close = float(df.at[j - 1, "close"]) if j - 1 >= 0 else None
                    low_j   = float(df.at[j, "low"])
                    close_j = float(df.at[j, "close"])

                    for L in manual_supply_levels:
                        # 조건: 이전 종가 > 매물대, 이번 저가 < 매물대, 이번 종가 > 매물대
                        if prev_close is not None and prev_close > L and low_j < L and close_j > L:
                            rebound_idx = j
                            break
    ...
    if dedup_mode.startswith("중복 제거"):
        ...
        while i < n:
            ...
            if row is not None:
                res.append(row)
                # ✅ anchor_idx 또는 lock_end 이후부터 탐색
                i = lock_end + 1 if lock_end is not None else i + lookahead
            else:
                i += 1
    else:
