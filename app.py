...
with c3:
    # ✅ 시작 = 어제, 종료 = 오늘  (date 타입으로 지정하여 API 범위 안정화)
    default_start = (datetime.today().date() - timedelta(days=1))
...
def simulate(df, rsi_mode, rsi_low, rsi_high, lookahead, thr_pct, bb_cond, dedup_mode,
    ...
    if rsi_mode != "없음" and bb_cond != "없음":
        ...
        # 1차/2차 모두 없음이면 신호 없음, 2차만 있으면 전체 후보
    ...
    def b1_pass(j):
        """자기 자신의 BB 기준으로 '50% 이상 진입'한 양봉인지 판정"""
        if not is_bull(j):
            return False
        if bb_cond == "상한선":
    ...
    while i < n:
        if i in base_sig_idx:
            ...
            if sec_cond == "양봉 2개 연속 상승":
                ...
                # ✅ i 루프에 종속되지 않도록 전용 탐색 루틴으로 신호 결정
                # B1: 전체 구간에서 '50% 이상 진입' 양봉 첫 캔들
                ...
                # B2, B3: B1 이후 '양봉 2개' (연속성 무관), B1 이후 lookahead 이내에서만 탐색
                ...
                # T: B3 이후 '종가 ≥ B1 종가'인 첫 캔들 (여기서부터 신호 시작)
                T_idx = None
            ...
            # ✅ 목표 미도달 시: 마지막 종가로 판정하되, 표의 최종수익률은 성공 시 '목표%' 고정
            ...
            if not first_hit.empty:
                ...
                end_time = hit_time
                end_close = target_price  # 오버슈팅 방지: 체결가를 목표가로 고정
                result = "성공"
                final_ret = thr
            else:
                if final_ret >= thr:
