...
try:
    ...
    pnl_colors = np.where(pnl_vals > 0, "red", np.where(pnl_vals < 0, "blue", "gray")).tolist()
    pnl_cd = np.column_stack([pnl_vals, pnl_colors])  # shape (N, 2), dtype=object OK
...
fig.add_trace(go.Candlestick(
    ...
    for col, name, color, dash, width in [
        ("BB_up",  "BB 상단", "#FFB703", "solid", 1.4),
        ("BB_low", "BB 하단", "#219EBC", "solid", 1.4),
        ("BB_mid", "BB 중앙", "#8D99AE", "dot",   1.1),
    ]:
    ...
    # RSI 보조축 (시각화만, 툴팁은 동일 규칙 유지 위해 끔)
    fig.add_trace(go.Scatter(
    ...
    with chart_box:
        ...
        with top_l:
            ...
            # 매수가 입력창 폭 축소 (종목 선택 UI와 유사)
            st.markdown(
                "<style>div[data-testid='stNumberInput'] {width: 220px !important;}</style>",
                unsafe_allow_html=True
            )

        with top_r:
            st.markdown("<div style='margin-top:6px'></div>", unsafe_allow_html=True)  # 버튼 아래로 살짝 내림
    ...
    def _summarize(df_in: Optional[pd.DataFrame]):
    ...
    if res is None or res.empty:
        ...
        for coln in ["성공기준(%)", "최종수익률(%)", "최저수익률(%)", "최고수익률(%)"]:
            if coln in tbl:
                tbl[coln] = tbl[coln].map(lambda v: f"{v:.2f}%" if pd.notna(v) else "")
