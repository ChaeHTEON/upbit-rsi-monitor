...
# ===== CCI 하단 차트 (row2) =====
        ...
        fig.update_layout(
            ...
            # ✅ row=2는 CCI 패널 → CCI가 맞음
            yaxis2=dict(title=f"CCI({int(cci_window)})", autorange=True,  fixedrange=False, title_standoff=10),
            # ✅ row=3는 RSI 패널 → RSI(13) 이동
            yaxis3=dict(title="RSI(13)", range=[0, 100], autorange=False, fixedrange=False, title_standoff=10),
            # ✅ 거래량, MACD도 동일 간격으로 정렬(라벨 밀림 방지)
            yaxis4=dict(title="거래량", autorange=True, fixedrange=False, title_standoff=10),
            yaxis5=dict(title="MACD", autorange=True, fixedrange=False, title_standoff=10),
            uirevision=_uirev,
            hovermode="closest")
