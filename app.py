...
def simulate(df, rsi_mode, rsi_low, rsi_high, lookahead, thr_pct,
    ...
    if rsi_mode == "없음":
        ...
        rsi_idx = sorted(set(df.index[df["RSI13"] <= rsi_low]) |
                         set(df.index[df["RSI13"] >= rsi_high]))
    elif rsi_mode == "과매도 기준":
        rsi_idx = df.index[df["RSI13"] <= rsi_low].tolist()
    else:  # 과매수 기준
        rsi_idx = df.index[df["RSI13"] >= rsi_high].tolist()
    ...
    def bb_ok(i):
        ...
        if bb_cond == "상한선":
            return pd.notna(up) and (close_i > up)
        if bb_cond == "하한선":
            return pd.notna(lo) and (close_i <= lo)
        if bb_cond == "중앙선":
            if pd.isna(mid): return False
            return abs(close_i - mid) <= 0.1 * (up - lo)
        return False
    ...
    # 보조 함수
    def is_bull(idx): return df.at[idx, "close"] > df.at[idx, "open"]
    ...
    while i < n:
        ...
        if sec_cond == "BB 기반 첫 양봉 50% 진입":
            ...
            for j in range(i + 1, n):
                if is_bull(j) and not pd.isna(df.at[j, "close"]):
                    B1_idx, B1_close = j, df.at[j, "close"]
                    break
            if B1_idx is None:
                i += 1
                continue
        ...
        window = df.iloc[anchor_idx+1:end_idx+1]  # ✅ N봉 정확히 포함
        end_time = df.at[end_idx, "time"]
        end_close = df.at[end_idx, "close"]
        ...
        hits = window[window["close"] >= target_price]
        if not hits.empty:
        ...
        res.append({
            ...
            "기준시가": int(base_price), "종료가": end_close,
            "RSI(13)": round(df.at[i, "RSI13"], 1) if pd.notna(df.at[i, "RSI13"]) else None,
            "BB값": round(df.at[i, "BB_low"], 1) if bb_cond == "하한선" else None,
...
try:
    ...
    res = simulate(df, rsi_mode, rsi_low, rsi_high,
                   lookahead, threshold_pct, bb_cond,
                   dup_mode, minutes_per_bar, market_code,
                   bb_window, bb_dev, sec_cond)
    st.dataframe(res)
