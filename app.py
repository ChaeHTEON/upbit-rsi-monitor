...
# -----------------------------
def simulate(
    df, rsi_mode, rsi_low, rsi_high, lookahead, thr_pct,
    bb_cond, dedup_mode, minutes_per_bar, market_code,
    bb_window, bb_dev, sec_cond="없음"
):
    """
    규칙(요약)
    - 기준가: 기본은 '첫 신호봉(i)의 종가', 단 'BB 기반 첫 양봉 50% 진입'일 때는 T(조건 d)의 종가.
    - 성공: 기준가 * (1 + thr%) 이상 '최초 도달 봉'에서 조기 종료.
    - 실패/중립: 앵커 이후 정확히 N(=lookahead)번째 봉에서 종료.
    - N개 카운트는 앵커 다음 봉부터 시작하여 N개를 딱 채움(슬라이싱 .iloc[anchor+1 : end+1]).
    """
    ...
    if rsi_mode == "없음":
        ...
        rsi_idx = sorted(
            set(df.index[df["RSI13"] <= float(rsi_low)].tolist()) |
    ...
    if rsi_mode != "없음" and bb_cond != "없음":
        ...
        # 1차 조건이 완전 '없음'인데 2차 조건만 사용할 때를 위한 안전장치
        base_sig_idx = list(range(n)) if sec_cond != "없음" else []

    # ---------- 4) 보조 함수 ----------
    def is_bull(k: int) -> bool:
        return float(df.at[k, "close"]) > float(df.at[k, "open"])

    def b1_pass(j: int) -> bool:
        """BB 기준 50% 상향 진입 판단"""
        if not is_bull(j):
            return False
        if bb_cond == "상한선":
        ...
        # (o < ref)면 ref까지의 50% 이상 종가가 올라와야 하고,
        # 이미 ref 이상에서 시작했으면 종가가 ref 이상이면 통과
    ...
    while i < n:
        ...
        if sec_cond == "양봉 2개 연속 상승":
            # i+1, i+2가 '연속 양봉'이면서 두 번째가 첫 번째보다 상승
            if i + 2 < n:
        ...
        # 중복 제거 모드면 이번 앵커의 N번째 봉 다음으로 점프
